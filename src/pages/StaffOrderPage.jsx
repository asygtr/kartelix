          ...
          <section className="app-panel p-6 app-collapse-panel">
            ...
            <div className="mt-5" ref={searchBlockRef}>
              <PageSearchBar
                ...
                getResultSecondary={(item) => canSeePrices ? `${formatArticleLabel(item.article_code, item.article_no)}${item.renk ? ` / ${item.renk}` : ''} / ${Number(item.bir_kg_satis_fiyati || 0).toFixed(2)}` : `${formatArticleLabel(item.article_code, item.article_no)}${item.renk ? ` / ${item.renk}` : ''}`}
                ...
                <div className="mt-4 space-y-2">
                  {selectedOrderDetail.items?.map((item) => (
                    <div key={item.id} className="app-panel px-4 py-3 text-sm">
                      <div className="font-semibold text-[color:var(--app-text)]">{item.mamul_adi}</div>
                      <div className="mt-0.5 text-[color:var(--app-text-muted)]">{formatArticleLabel(item.article_code, item.article_no)}</div>
